...
path = 'C:/tensorflow-vgg16-train-and-test/picture/'
w = 224
h = 224
c = 3
...
def build_network(height, width, channel):
    ...
    correct_times_in_batch = tf.reduce_sum(tf.cast(correct_prediction, tf.int32))  # 预测正确的结果
...
def train_network(graph, batch_size, num_epochs):
    ...
    with tf.Session() as sess:
        ...
        for epoch_index in range(num_epochs):
            ...
            if epoch_index % epoch_delta == 0:  # 每2个epoch后输出结果
                ...
                for i in range(3):
                    ...
                    total_cost_in_test_set += mean_cost_in_batch

                acy_on_test = total_correct_times_in_test_set / float(total_batches_in_test_set)
                acy_on_train = total_correct_times_in_train_set / float(total_batches_in_train_set)
                print(
                    'Epoch - {:2d}, train_acc:{:6.2f}({}/{}),train_loss:{:6.2f}, test_acc:{:6.2f}({}/{}),test_loss:{:6.2f}'.format(
                    epoch_index,
                    acy_on_train,
                    total_correct_times_in_train_set,
                    total_batches_in_train_set,
                    total_cost_in_train_set,
                    acy_on_test,
                    total_correct_times_in_test_set,
                    total_batches_in_test_set,
...
def main():
    ...
    train_network(g, batch_size, num_epochs)
